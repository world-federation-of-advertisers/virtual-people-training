...
std::vector<std::tuple<std::string, std::string, std::string>> ParseConfig(std::string path) {
    ...
    for (int testIndex = 0; testIndex < config.tests_size(); testIndex++) {
        ...
        for (int testCaseIndex = 0; testCaseIndex < it.test_cases_size(); testCaseIndex++) {
...
class IntegrationTestParamaterizedFixture : public ::testing::TestWithParam<std::tuple<std::string, std::string, std::string>> {
    protected:
...
TEST_P(IntegrationTestParamaterizedFixture, Test) { 
    ...
    CompiledNode output;
    absl::Status outputStatus = ReadTextProtoFile(std::get<1>(targetPair), output);

    CompiledNode golden;
    absl::Status goldenStatus = ReadTextProtoFile(std::get<2>(targetPair), golden);

    ASSERT_TRUE(diff.Equals(output, golden));
}

INSTANTIATE_TEST_SUITE_P(IntegrationTest, IntegrationTestParamaterizedFixture,
    ::testing::ValuesIn(ParseConfig("src/test/cc/wfa/virtual_people/training/model_compiler/test_data/config.textproto")),
    [](const ::testing::TestParamInfo<IntegrationTestParamaterizedFixture::ParamType>& info) {
